...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the EwiseSetitem opeation. Set items in a (non-compact) array using CUDA.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the ScalarSetitem opeation. Set items in a (non-compact) array using CUDA.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
...
void ScalarMaximum(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Set entries in out to be the max of corresponding entry in a and the scalar val.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseEq(const CudaArray& a, const CudaArray& b, CudaArray* out) {
  /**
   * Set entries in out to 1 if correspondings entires in a and b are equal, else 0.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void ScalarEq(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Set entries in out to 1 if corresponding entry in a and the scalar val are equal, else 0.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void EwiseGe(const CudaArray& a, const CudaArray& b, CudaArray* out) {
  /**
   * Set entries in out to 1 if corresponding entry in a >= entry in b, else 0.
   */
  CudaDims dim = CudaOneDim(out->size);
...
void ScalarGe(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * Set entries in out to 1 if corresponding entry in a >= the scalar val, else 0.
   */
  CudaDims dim = CudaOneDim(out->size);
