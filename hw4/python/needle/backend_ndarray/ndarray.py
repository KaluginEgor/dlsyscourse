...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple(
            (idx.stop - idx.start - 1) // idx.step + 1 for idx in idxs
        )
        new_strides = tuple(stride * idx.step for stride, idx in zip(self.strides, idxs))
        new_offset = self._offset + sum(
            idx.start * stride for stride, idx in zip(self.strides, idxs)
        )
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        fliped_strides = list(self.strides)
        if isinstance(axes, int):
            axes = [axes]
        offset = 0
        for axis in axes:
            fliped_strides[axis] = -fliped_strides[axis]
            offset += (self.shape[axis] - 1) * self.strides[axis]
        return NDArray.make(
            self.shape, strides=tuple(fliped_strides), device=self.device, handle=self._handle, offset=offset
        ).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for idx, axis in enumerate(axes):
            padded_shape[idx] += sum(axis)
        padded_array = full(padded_shape, 0, dtype=self.dtype, device=self.device)
        slices = tuple(
            [slice(i, padded_shape[idx] - j) for idx, (i, j) in enumerate(axes)]
        )
