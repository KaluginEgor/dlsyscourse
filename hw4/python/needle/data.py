...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        self.images = np.concatenate([batch[b'data'] for batch in batches])
        self.images = np.reshape(self.images, (self.images.shape[0], 3, 32, 32)) / 255
        self.labels = np.concatenate([batch[b'labels'] for batch in batches])
        ### END YOUR SOLUTION
...
def batchify(data, batch_size, device, dtype):
    ...
    batchified_data = np.array(data_subset, dtype=dtype)
    batchified_data = batchified_data.reshape((batch_size, nbatch)).T
...
def get_batch(batches, i, bptt, device=None, dtype=None):
    ...
    """
    ### BEGIN YOUR SOLUTION
    remaining_len = batches.shape[0] - i - 1
    remaining_bptt = min(bptt, remaining_len)
    # assert(remaining_bptt >= 2)

    data_start = i
    data_stop = i + remaining_bptt

    target_start = data_start + 1
    target_stop = data_stop + 1

    data_np = batches[data_start:data_stop, :]
    target_np = batches[target_start:target_stop, :]
